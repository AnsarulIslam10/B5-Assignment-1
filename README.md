...
The `keyof` operator in typescript is used to extract new types from an existing object type's keys. for example -
...
```
in this example we can see that by using the `keyof` operator we did'nt have to declear the types of Owner manually. insted just by using the `keyof` operator we are able to extract all types from Vehicle and use it for the Owner.